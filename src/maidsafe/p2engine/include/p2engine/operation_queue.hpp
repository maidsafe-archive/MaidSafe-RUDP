#ifndef operation_queue_h__
#define operation_queue_h__

#include "p2engine/push_warning_option.hpp"
#include "p2engine/config.h"
#include <map>
...
namespace p2engine
{
	...
	{
		typedef asio::detail::handler_alloc_traits handler_alloc_traits;
		typedef asio::detail::raw_handler_ptr raw_handler_ptr;
		typedef asio::detail::handler_ptr handler_ptr;
	...
	public:
		...
		void get_descriptors(Descriptor_Set& descriptors)
		{
			...
			while (i != operations_.end())
			{
				...
				{
					error_code ec(error::fd_set_failure);
	...
	private:
		...
		public:
			...
			bool perform(const error_code& result)
			{
				result_ = result;
		...
		public:
			...
			static bool do_perform(op_base* base,
				error_code& result, std::size_t& bytes_transferred)
			{
			...
			static void do_complete(op_base* base,
				const error_code& result, std::size_t bytes_transferred)
			{
		...
		typedef boost::unordered_map<Descriptor, op_base*> operation_map;
