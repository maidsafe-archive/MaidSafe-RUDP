...
int main(int argc, char** argv) {
  ...
  try {
    ...
    boost::filesystem::path pmids_path(
        GetPathFromProgramOption("pmids_path", &variables_map, false, true));
    if (fs::exists(pmids_path, error_code)) {
      ...
      if (static_cast<uint32_t>(peer_identity_index) >= all_pmids.size() ||
        ...
        std::cout << "ERROR : index exceeds fob pool -- pool has " << all_pmids.size()
                  << " fobs, while peer_identity_index is " << peer_identity_index << std::endl;
        return 0;
      }
    }

    std::string peer(variables_map.at("peer").as<std::string>());
    maidsafe::rudp::test::RudpNode rudp_node(all_pmids, identity_index, peer_identity_index, peer);
    rudp_node.Run();
